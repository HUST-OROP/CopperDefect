...
class DepthBBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 depth_predictor_cfg = dict(type="Linear"),
        ...
        super(DepthBBoxHead, self).__init__(init_cfg)
        assert with_cls or with_reg or with_depth
        ...
        self.depth_predictor_cfg = depth_predictor_cfg
        ...
        if self.with_depth:
            ...
            out_dim_depth = num_classes + 1
            self.fc_depth = build_linear_layer(
                self.depth_predictor_cfg,
                in_features = in_channels,
                out_features = out_dim_depth
            )
        ...
        if init_cfg is None:
            ...
            if self.with_depth:
                self.init_cfg +=[
                    dict(
                        type='Normal', std=0.001, override=dict(name='fc_depth'))
                ]
    ...
    def forward(self, x):
        ...
        depth_pred = self.fc_depth(x) if self.with_depth else None
        return cls_score,bbox_pred,depth_pred

    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
                           pos_gt_labels, pos_gt_depth,cfg):
        ...
        Args:
            ...
            pos_gt_depth (Tensor): Contains gt_depth for
                all positive samples, has shape (num_pos, ).
        ...
        depth = pos_bboxes.new_full((num_samples,),0,dtype=torch.long)

        bbox_targets = pos_bboxes.new_zeros(num_samples, 4)
        bbox_weights = pos_bboxes.new_zeros(num_samples, 4)
        if num_pos > 0:
            labels[:num_pos] = pos_gt_labels
            depth[:num_pos] = pos_gt_depth
        ...
        return labels, label_weights, bbox_targets, bbox_weights, depth
    ...
    def get_targets(self,
        ...
        Returns:
            ...
            Containing the following list of Tensors:
                ...
                - depth (list[Tensor],Tensor): Gt_depth for all
                  proposals in a batch, each tensor in list has
                  shape (num_proposals,) when `concat=False`, otherwise
                  just a single tensor has shape (num_all_proposals,).
        """
        ...
        pos_gt_depth_list = [res.pos_gt_depth for res in sampling_results]
        
        labels, label_weights, bbox_targets, bbox_weights,depth = multi_apply(
    ...
    @force_fp32(apply_to=('cls_score', 'bbox_pred',"depth_pred"))
    def loss(self,
             cls_score,
             bbox_pred,
             depth_pred,
        ...
        if depth_pred is not None:
            ...
            if pos_inds.any():
                if depth_pred.numel() >0:
                    pos_depth_pred = depth_pred[pos_inds.type(torch.bool),labels[pos_inds.type(torch.bool)]]
                    ##TODO:add parameters for depth regression
                    loss_depth_ = self.loss_depth(
                        pos_depth_pred,
                        depth[pos_inds.type(torch.bool)],
                        avg_factor=depth.size(0),
                        reduction_override=reduction_override
                    )
                    
                    if isinstance(loss_depth_, dict):
                        losses.update(loss_depth_)
                    else:
                        losses['loss_depth'] = loss_depth_
            else:
                losses['loss_depth'] = depth_pred[pos_inds].sum()
    ...
    def get_bboxes(self,
        ...
        if cfg is None:
            ...
            ## else
            det_depth = depth_pred[index][:,:-1]
            
            det_results = torch.cat([det_bboxes, det_depth], -1)
